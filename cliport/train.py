...
def main(cfg):
    ...
    last_checkpoint = last_checkpoint_path if os.path.exists(last_checkpoint_path) and \
                                              cfg['train']['load_from_last_ckpt'] else None
    ...
    if 'multi' in dataset_type:
        ...
        val_ds = RavensMultiTaskDataset(data_dir, cfg, group=task, mode='val', n_demos=n_val,
                                        augment=False)
    elif 'weighted' in dataset_type:
        train_ds = RavenMultiTaskDatasetBalance(data_dir, cfg, group=task, mode='train',
                                                n_demos=n_demos, augment=True)
        val_ds = RavenMultiTaskDatasetBalance(data_dir, cfg, group=task, mode='val', n_demos=n_val,
                                              augment=False)
    else:
        train_ds = RavensDataset(os.path.join(data_dir, '{}-train'.format(task)), cfg,
                                 n_demos=n_demos, augment=True)
        val_ds = RavensDataset(os.path.join(data_dir, '{}-val'.format(task)), cfg, n_demos=n_val,
                               augment=False)
