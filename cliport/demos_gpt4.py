...
from cliport.simgen_utils import (save_text,
                                  add_to_txt,
                                  extract_code,
                                  extract_dict,
                                  extract_assets,
...
def generate_feedback(prompt, max_tokens=2048, temperature=0.0, model="gpt-4",
                      assistant_prompt=None, interaction_txt=None):
    """ use GPT-4 API """
    params = {
        "model": model,
...
def llm_gen_env(cfg, model_output_dir):
    ...
    res, full_interaction = generate_feedback(task_prompt_text, temperature=cfg['gpt_temperature'],
                                              interaction_txt=full_interaction)

    # Extract dictionary for task name, descriptions, and assets
    task_def = extract_dict(res, prefix="new_task")
    exec(task_def, globals())

    full_interaction = add_to_txt(full_interaction, "================= Task and Asset Design!",
                                  with_print=True)
    pprint(new_task)
    ...
    if os.path.exists(f"{prompt_folder}/cliport_prompt_asset_template.txt"):
        full_interaction = add_to_txt(full_interaction, "================= Asset Generation!",
                                      with_print=True)
        ...
        asset_prompt_text = asset_prompt_text.replace("ASSET_STRING_TEMPLATE",
                                                      str(new_task["assets-used"]))

        res, full_interaction = generate_feedback(asset_prompt_text, temperature=0,
                                                  assistant_prompt=res,
                                                  interaction_txt=full_interaction)  # cfg['gpt_temperature']
    ...
    if os.path.exists(f"{prompt_folder}/cliport_prompt_api_template.txt"):
        ...
        res, full_interaction = generate_feedback(api_prompt_text, temperature=0,
                                                  assistant_prompt=res,
                                                  interaction_txt=full_interaction)  # cfg['gpt_temperature']
    ...
    if os.path.exists(f"{prompt_folder}/cliport_prompt_common_errors_template.txt"):
        ...
        errorbook_prompt_text = open(
            f'{prompt_folder}/cliport_prompt_common_errors_template.txt').read()
        errorbook_prompt_text = errorbook_prompt_text.replace("TASK_NAME_TEMPLATE",
                                                              new_task["task-name"])
        res, full_interaction = generate_feedback(errorbook_prompt_text, temperature=0.,
                                                  assistant_prompt=res,
                                                  interaction_txt=full_interaction)  # cfg['gpt_temperature']
    ...
    if os.path.exists(f"{prompt_folder}/cliport_prompt_code_split_template.txt"):
        ...
        res, full_interaction = generate_feedback(code_prompt_text, temperature=0.,
                                                  assistant_prompt=res,
                                                  interaction_txt=full_interaction)  # cfg['gpt_temperature']
...
def main(cfg):
    ...
    for trial_i in range(TOTAL_TRIALS):
        ...
        print("=========================================================")
        print(
            f"SYNTAX_PASS_RATE: {(SYNTAX_PASS_RATE / (trial_i + 1)) * 100:.1f}% RUNTIME_PASS_RATE: {(RUNTIME_PASS_RATE / (trial_i + 1)) * 100:.1f}% ENV_PASS_RATE: {(ENV_PASS_RATE / (trial_i + 1)) * 100:.1f}%")
        print("=========================================================")

        prompt_folder = f"prompts/{cfg['prompt_folder']}"
        if os.path.exists(
                f"{prompt_folder}/cliport_prompt_task_reflection.txt") and env_success_cnt >= 1:
            ...
            code_reflection_prompt_text = open(
                f"{prompt_folder}/cliport_prompt_task_reflection.txt").read()
            ...
            res, full_interaction = generate_feedback(code_reflection_prompt_text, temperature=0.,
                                                      interaction_txt=full_interaction)  # cfg['gpt_temperature']
            ...
            if task_reflection["add_to_the_task_list"] == 'True':
                ...
                if cfg['save_memory']:
                    ...
                    generated_task_code_path = os.path.join(cfg['prompt_data_path'],
                                                            'generated_task_codes.json')
                    generated_task_codes = json.load(open(generated_task_code_path))
                    generated_task_codes.append(new_task["task-name"] + ".py")
                    with open('cliport/generated_tasks/' + new_task["task-name"].replace("-",
                                                                                         "_") + ".py",
                              "w") as fhandle:
                        fhandle.write(code)
                    ...
                    generated_task_path = os.path.join(cfg['prompt_data_path'],
    ...
    save_stat(cfg, model_output_dir, env_names, SYNTAX_PASS_RATE / TOTAL_TRIALS,
              RUNTIME_PASS_RATE / TOTAL_TRIALS, ENV_PASS_RATE / TOTAL_TRIALS, DIVERSITY_SCORE)
