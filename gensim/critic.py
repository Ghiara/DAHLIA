...
class Critic:
    ...
    def error_review(self, new_task):
        ...
        if os.path.exists(
                f"{self.prompt_folder}/cliport_prompt_common_errors_template.txt") and "task-name" in new_task:
            self.chat_log = add_to_txt(self.chat_log, "================= Error Book Preview!",
                                       with_print=True)
            errorbook_prompt_text = open(
                f'{self.prompt_folder}/cliport_prompt_common_errors_template.txt').read()
            errorbook_prompt_text = errorbook_prompt_text.replace("TASK_NAME_TEMPLATE",
                                                                  new_task["task-name"])
            res = generate_feedback(errorbook_prompt_text, temperature=0.,
                                    interaction_txt=self.chat_log)  # cfg['gpt_temperature']
    ...
    def reflection(self, new_task, new_code, current_tasks=None):
        ...
        if os.path.exists(f"{self.prompt_folder}/cliport_prompt_task_reflection.txt"):
            ...
            self.chat_log = add_to_txt(self.chat_log, "================= Code Reflect!",
                                       with_print=True)
            ...
            code_reflection_prompt_text = open(
                f"{self.prompt_folder}/cliport_prompt_task_reflection.txt").read()
            ...
            code_reflection_prompt_text = code_reflection_prompt_text.replace("TASK_CODE_TEMPLATE",
                                                                              str(new_code))
            ...
            res = generate_feedback(code_reflection_prompt_text, temperature=0.4,
                                    interaction_txt=self.chat_log, n=int(
                    self.cfg['reflection_agreement_num']))  # cfg['gpt_temperature']
            all_add_to_the_task_list_flag = True
            ...
            save_text(self.model_output_dir, new_task['task-name'] + "_reflection_output",
                      str(task_reflection))
