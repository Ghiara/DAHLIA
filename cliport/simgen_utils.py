...
def save_stat(cfg, output_dir, env_names, syntax_rate, run_rate, env_success_rate,
    ...
    print("=========================================================")
    print(
        f"TOTAL SYNTAX_PASS_RATE: {syntax_rate * 100:.1f}% RUNTIME_PASS_RATE: {run_rate * 100:.1f}% ENV_PASS_RATE: {env_success_rate * 100:.1f}% DIVERSITE SCORE: {diversity_score:.3f}")
    print("=========================================================")
    ...
    with open(os.path.join(output_dir, "eval_results.csv"), "w") as f:
        ...
        for col, stat in zip(["syntax", "runtime", "env. completion", "diversity"],
