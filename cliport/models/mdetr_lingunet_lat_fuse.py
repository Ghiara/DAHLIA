...
class MDETRLingUNetLat_fuse(nn.Module):
    ...
    def __init__(self, input_shape, output_dim, cfg, device, preprocess):
        ...
        encoder_layer = TransformerEncoderLayer(d_model=256, nhead=8, dim_feedforward=2048,
                                                dropout=0.1, activation='relu',
                                                normalize_before=False)
        self.encoder = TransformerEncoder(encoder_layer, 6, None)
        mdter_checkpoint = torch.load(
            '/home/yzc/shared/project/GPT-CLIPort/ckpts/mdetr_pretrained_resnet101_checkpoint.pth',
    ...
    def encode_text(self, x):
        with torch.no_grad():
            tokenized = self.tokenizer.batch_encode_plus(x, padding="longest",
    ...
    def forward(self, x, lat, l):
        ...
        with torch.no_grad():
            ...
            img_memory, img_memory_all = self.encoder(src, src_key_padding_mask=mask,
                                                      pos=pos_embed)
...
class TransformerEncoderLayer(nn.Module):
    ...
    def forward_post(
        ...
        src2 = \
        self.self_attn(q, k, value=src, attn_mask=src_mask, key_padding_mask=src_key_padding_mask)[
            0]
