...
class TwoStreamTransportLangFusion(Transport):
    ...
    def forward(self, inp_img, p, lang_goal, softmax=True):
        ...
        if type(inp_img) is not torch.Tensor:
            ...
            in_tens = torch.from_numpy(in_data).to(dtype=torch.float,
                                                   device=self.device)  # [B W H 6]
        else:
            in_data = inp_img
            in_tens = in_data

        in_tensor = torch.nn.functional.pad(in_tens, tuple(self.padding[[2, 1, 0]].reshape(-1)),
                                            mode='constant')
        ...
        for i in range(len(in_tensor)):
            ...
            crop = crop[:, :, int(pv[0] - hcrop):int(pv[0] + hcrop),
                   int(pv[1] - hcrop):int(pv[1] + hcrop)]

            in_tensors.append(in_tensor_i)
            crops.append(crop)

        logits, kernels = self.transport(torch.cat(in_tensors, dim=0), torch.cat(crops, dim=0),
                                         lang_goal)  # crops.shape:(8, 36, 6, 64, 64)
