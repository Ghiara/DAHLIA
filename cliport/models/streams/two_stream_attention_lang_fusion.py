...
class TwoStreamAttentionLangFusion(Attention):
    ...
    def _build_nets(self):
        ...
        self.attn_stream_one = stream_one_model(self.in_shape, 1, self.cfg, self.device,
                                                self.preprocess)
        self.attn_stream_two = stream_two_model(self.in_shape, 1, self.cfg, self.device,
                                                self.preprocess)
    ...
    def forward(self, inp_img, lang_goal, softmax=True):
        ...
        if type(inp_img) is not torch.Tensor:
            ...
            in_tens = torch.from_numpy(in_data.copy()).to(dtype=torch.float,
        ...
        # [B W H 6]
        in_tens = torch.nn.functional.pad(in_tens, tuple(self.padding[[2, 1, 0]].reshape(-1)),
                                          mode='constant')
